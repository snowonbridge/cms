...
class Autotask extends Controller
{
    ...
    public function index()
    {
        ...
        foreach ($crontabList as $crontab)
        {
            ...
            try {
                if ($crontab['type'] == 'url') {
                    Log::record('crontab run url'.var_export($crontab,true));
                    if (substr($crontab['content'], 0, 4) == "http") {
                        // 远程异步调用URL
                        Log::record('crontab ru');
                        Http::sendAsyncRequest($crontab['content']);
                    } else {
                        //本地项目URL
                        exec('nohup /usr/local/php/bin/php ' . ROOT_PATH . 'public/index.php ' . $crontab['content'] . ' >> ' . $logDir . date("Y-m-d") . '.log 2>&1 &');
                        Log::record('crontab run log nohup /usr/local/php/bin/php ' . ROOT_PATH . 'public/index.php ' . $crontab['content'] . ' >> ' . $logDir . date("Y-m-d") . '.log 2>&1 &' );
                    }
                }
