define(['jquery', 'bootstrap', 'backend', 'table', 'form','bootstrap-editable','select2'], function ($, undefined, Backend, Table, Form,editable,select2,E) {
...
//Table.api.formatter.label (value, row, index)
        ...
        edit: function () {
            ...
            $("body").on("input","#game_table input",function (e) {
                ...
                var index = $(this).parents('tr').data("value");
                var game = $("#c-game").val();
                var gameList = JSON.parse(game);
                gameList[index][col_name] = val;
            ...
            $("body").on("click","#add_game_row",function (e) {
                ...
                var index =  $("#game_table tbody tr").length;
                $("#game_table tbody ").append("<tr id='game-"+index+"' data-value='"+index+"'>"+html_tr+"</tr>");
                var game = $("#c-game").val();
                var gameList = JSON.parse(game);
            ...
            $("body").on("click","#del_game_row",function (e) {
                var index =  $("#game_table tbody tr").length-1;
                $("#game_table tbody tr:last").remove();
                var game = $("#c-game").val();
                var gameList = JSON.parse(game);
                var counts = gameList.length;

                delete gameList[index] ;
                gameList.length = counts-1;
                $("#c-game").val(JSON.stringify(gameList));
            })
            //notify_way
