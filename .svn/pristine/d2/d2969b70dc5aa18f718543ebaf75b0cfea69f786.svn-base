        ...
        api: {
            ...
            bindevent: function (table) {
              ...
              $(toolbar).on('click', Table.config.multibtn, function () {
                ...
                // 分时间统计
                $(toolbar).on('click', Table.config.statbtn, function () {
                    Table.api.stat( table, this);
                });
                $(toolbar).on("click", Table.config.sendbtn, function (e) {
                    var ids = Table.api.selectedids(table);
                    var that = this;
            ...
            multi: function (action, ids, table, element) {
                ...
                var  reg=/^http[s]?:\/\//;
                var datatype = reg.test(url) ? 'jsonp' : 'json';
            ...
            plupload: function (element, onUploadSuccess, onUploadError) {
                ...
                $(element, Upload.config.container).each(function () {
                    ...
                    Upload.list[id] = new Plupload.Uploader({
                        ...
                        url: url,
                        multipart_params:{"act":Config.jsname.replace('backend/','')},
                        init: {
                            PostInit: function () {
...
define('form',['jquery', 'bootstrap', 'upload', 'validator','moment'], function ($, undefined, Upload, Validator, Moment) {
    var Form = {
        config: {
        },
        events: {
            ...
            selectpage: function (form,format_item) {
                //绑定selectpage元素事件
                if ($(".selectpage", form).size() > 0) {
                  var config = {
                    source: 'ajax/selectpage',
                  };
                  if(format_item!==undefined && $.isFunction(format_item)){
                    config =$.extend({},config,{format_item:format_item});
                  }
                    require(['selectpage'], function () {
                        $('.selectpage', form).selectPage(config);
                    });
        ...
        api: {
            submit: function (form, success, error, submit) {
                ...
                var  reg= new RegExp('^http[s]?:\/\/'+Config.gate_host);
                var dataType = reg.test(url) ? 'jsonp' : 'json';
                //修复当存在多选项元素时提交的BUG
                var params = {};
            ...
            bindevent: function (form, success, error, submit, item_format) {
                ...
                events.selectpage(form, item_format);
