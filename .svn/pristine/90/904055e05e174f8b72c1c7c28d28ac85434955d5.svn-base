...
(SELECT SUM(profit_money) AS profit_money_sum,sid,agent_id,id FROM agent_profit_log WHERE create_time BETWEEN {$start_time} AND {$end_time} GROUP BY sid,agent_id ) tmp
ON tmp.agent_id=agent_details.agent_id AND tmp.sid = agent_details.sid
WHERE agent_details.player_count > (CASE WHEN agent_details.agent_level=2 THEN {$agent_level_rule['up'][1]['member']} WHEN agent_details.agent_level=3 THEN {$agent_level_rule['up'][2]['member']} ELSE agent_details.player_count END)
OR tmp.profit_money_sum > (CASE WHEN agent_details.agent_level=2 THEN {$agent_level_rule['up'][1]['profit']} WHEN agent_details.agent_level=3 THEN {$agent_level_rule['up'][2]['profit']} ELSE tmp.profit_money_sum END)
...
LIMIT {$offset},{$limit};";
            ...
            foreach($list as $v){
                ...
                if($v['agent_level']==3){
                    $down_member = $agent_level_rule['up'][1]['member']<$v['player_count'];
                    $down_profit = $agent_level_rule['up'][1]['profit']<$v['profit_money_sum'];
