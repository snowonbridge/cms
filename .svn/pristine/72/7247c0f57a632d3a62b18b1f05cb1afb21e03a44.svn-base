...
{
    const SUCCESS = 1;//正常
    const CODE_ERR_PARAM = -109; //参数错误
    const CODE_ERR_AUTH = -110; // 权限不足
    const CODE_USER_NO_EXISTS = -120;//用户不存在
    const OPERATE_EXCEPTION = -238;//操作出现未知异常

	const CODEERRPARAM = -109; //参数错误
    const CODEERRAUTH = -110; // 权限不足
	...
	const CODEUSERNOEXISTS = -120;//用户不存在    const CODEUSERAUTHFAILED = -121;//在线效验失败
    ...
    const CODETOOLREPEATUSING = -149;//重复使用
    const CODEGAMDIDERR = -150;//游戏Id错误
    const CODE_AGENT_NOEXISTS = -151;//代理用户不存在
