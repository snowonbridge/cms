    ...
    public function edit($ids = NULL)
    {
        ...
        if ($this->request->isPost())
        {
            ...
            if ($params)
            {
                ...
                $img_url = $params['imgs']=!empty($params['imgs'])?$this->getImgUrl($params['imgs']):'';
                $gift_content = json_decode($params['gift_content'],true);
                foreach ($gift_content as &$gift)
                {
                    if(count($gift['from']) != count($gift['from'],COUNT_RECURSIVE))
                    {
                        foreach ($gift['from'] as &$item)
                        {
                            if(isset($item['id']) && $item['id'] == 120 &&  isset($item['url']))
                                $item['url']=$img_url;
                        }
                    }
                }
                ...
                $params['gift_content'] = json_encode($gift_content);
