...
use app\admin\library\BankList;
use app\admin\model\AgentBalanceLog;
...
class Agentsettlementorder extends Backend
{
    ...
    public function bank_type()
    {
        ...
        return json(['bank_type'=>BankList::bankCardName($account)]);
    }
    /**
     * 查看
     */
    ...
    public function add()
    {
        if ($this->request->isPost())
        {
            ...
            if ($params)
            {
                ...
                {
                    $this->error('提现申请金额不能小于0');
                }
                if(floor($agentInfo['money']*100) < floor($params['pay_money']*100))
                {
                ...
                if(Cache::store('redis')->handler()->setnx("rPay|{$params['admin_id']}|{$params['agent_id']}",1))
                {
                    ...
                    Db::startTrans();
                    try{
                        $data['total_balance'] = ($agentInfo['total_balance']) * 100 ;
                ...
                try
                {
                    ...
                    if($order)
                    {
                        ...
                        {//审核通过
                            ...
                            }elseif($params['pay_status'] == 4){//改为提现失败

                                unset($params['pay_money'],$params['settlement_money'],$params['transfer_charge']);//todo

                                model("AgentSettlementOrder")->where(['apply_no'=>$params['apply_no']])->update($params);
                                $this->success("操作成功");
                            }else{
                                $this->error("审核通过的订单只能更改提现状态");
                            }
...
//            $this->error('添加日志失败');
            ...
            * 账户金额流水日志
             */
            $balance_log['agent_id'] = $params['agent_id'];
            $balance_log['flag'] = 2;//1加 2减',
            $balance_log['log_type'] = 3;//（0无 1房卡 2钻石 3管理后台操作）
            $balance_log['op_money'] = $params['pay_money'];
            $balance_log['left_money'] = $data['money'];
            $balance_log['remark'] = "后台管理员对代理ID{$params['agent_id']}的成功提现操作";
            $balance_log['create_time'] = time();
            model('AgentBalanceLog')->save($balance_log);
