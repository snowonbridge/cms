        ...
        if($uid)
        {
            ...
            while($i <$counts)
            {
                try{
                    ...
                    if(!isset($response['response_body']['data']['gift_list']))
                    {
                        $response['response_body']['data']['gift_list']=[];
                    }
                    $result[] = $response;
                    if( $response['response_body']['code'] != 0)
                    {
                        $this->assign('error',$response['response_body']['msg']) ;
                    }
                }catch (Exception $e)
                {
                    $error_flag = 1;
